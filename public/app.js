...
async function applyDefaultSelectors() {
  const defaults = await api("/api/default-selectors");
  const form = el("recordForm");
  if (!form.usernameField.value) form.usernameField.value = defaults.usernameField[0] || "";
  if (!form.passwordField.value) form.passwordField.value = defaults.passwordField[0] || "";
  if (!form.submitSelector.value) form.submitSelector.value = defaults.submitSelector[0] || "";

  let extra = {};
  if (form.extraFieldSelectors.value.trim()) {
    extra = JSON.parse(form.extraFieldSelectors.value);
  }
  const hasTotpMapping = Object.values(extra).includes("totp");
  if (!hasTotpMapping && defaults.totpField?.length) {
    extra[defaults.totpField[0]] = "totp";
    form.extraFieldSelectors.value = JSON.stringify(extra, null, 2);
  }
  msg("Default selectors applied (you can edit them before saving).");
}
...
el("applyDefaultsBtn").onclick = async () => {
  try {
    await applyDefaultSelectors();
  } catch (error) { msg(error.message); }
};
